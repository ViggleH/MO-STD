...
clear; clc; close all;
diary('.\Data\Conic-Single\output.txt');
t_total = tic;
...
nVars = 8; % Number of decision variables (adjust as needed)
lb = [200, -40, 40, 40, 0, 10, 70, 0]; % Lower bounds for decision variables
ub = [500, 40, 100, 160, 5, 400, 120, 2.5]; % Upper bounds for decision variables
...
%popSizes = [16384, 4096, 1024, 256, 64]*8;
popSizes = [256*8];
...
for i = 1:length(rayCounts)
    ...
    tempTable = array2table([xPareto, repmat(currentRayCount, size(xPareto,1),1), fPareto], ...
        'VariableNames', {'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7', 'x8', 'rayCount', 'Objective1', 'Objective2', 'Objective3'});
    ...
    tempTable = array2table([xPareto, ...
        ...
        'VariableNames', {'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7', 'x8', 'rayCount', 'Objective1', 'Objective2', 'Objective3', 'ComputingTime'});
...
title('Pareto Fronts at Different $N_{\mathrm{rays}}$ (Conic Lens, FlexyDos3D)', Interpreter='latex');
...
outputFolder = fullfile('Data', 'Conic-Single');
